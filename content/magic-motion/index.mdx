...
import { InverseScaleFormula } from "./components/InverseScaleFormula";
...
The first thing that I tried was to calculate the inverse scale _once_, just before the parent is about to animate:
...
So if the parent is animating from `scaleX: 2` to `scaleX: 1`, then the child will be animating from `scaleX: 1 / 2` to `scaleX: 1`. My thinking was that as long as the timing of the scale correction is the same as the parent animation, this approach should work. Here's what this approach looked like:

```
incorrect scale correction animation
```

It's doing _something_, but the text is still clearly changing size throughout the animation.
