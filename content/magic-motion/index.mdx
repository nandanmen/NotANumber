...
By doing all of the steps fast enough, we get...

Voila! Magical layout animations.

## Animating Size

So far we've only used FLIP to animate a change in _position_. Can we do the same thing but for _size_?

In this section, we'll try to replicate the following animation where the square stretches to fill the whole container:
...
Thankfully, the `.getBoundingClientRect()` method we used to measure the square also returns the `width` and `height` of the element.

```ts
const { width, height } = squareRef.current.getBoundingClientRect();
```

We can use this to measure the square's size before and after the layout change:

```
first and last visual
```

### Inverting Size Changes

To invert the size change, we use `scale` instead of `translate`:
...
And instead of animating the scale to zero, we animate the scale to one (if we animate to zero, the element will disappear altogether):
...
Now that we can animate both position and size, we're surely done, right?

## Consolidating Size with Position

What happens when we try to animate both size and position together?

```
broken position and size animation
```

Hmm, that looks a little off. Ideally, the animation should look seamless like this:

```
working position and size animation
```

Let's try to debug this. Here I've broken down the animation to its four FLIP components. Can you spot which step is broken?

```
broken animation split up into phases
```

<Quiz label="broken-flip-step" answer="inverse">

<Quiz.Question>Which step is broken?</Quiz.Question>

<Quiz.Options>
  <Quiz.Option label="first">First</Quiz.Option>
  <Quiz.Option label="last">Last</Quiz.Option>
  <Quiz.Option label="inverse">Inverse</Quiz.Option>
  <Quiz.Option label="play">Play</Quiz.Option>
</Quiz.Options>

<Quiz.Tip htmlFor="first">
  Hmm, that's not quite it. Try taking a look at the different phases again.
</Quiz.Tip>
<Quiz.Tip htmlFor="last">
  Hmm, that's not quite it. Try taking a look at the different phases again.
</Quiz.Tip>
<Quiz.Tip htmlFor="inverse">
  That's right! We didn't move the square back to its original position, causing
  the square to look like it jumped a bit.
</Quiz.Tip>
<Quiz.Tip htmlFor="play">
  Hmm, that's not quite it. Try taking a look at the different phases again.
</Quiz.Tip>

</Quiz>

What's going wrong in the <Quiz.Spoiler htmlFor="broken-flip-step">inverse</Quiz.Spoiler> phase, and how do we fix it?

### Size Affects Distance Travelled

When an element changes size, the distance that it travels changes as well. Specifically, if an element gets smaller, it has to travel _further_, and if it gets bigger, it has to travel _nearer_.
...
```
show inverse coming up a bit short
```
