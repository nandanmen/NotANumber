...
In reality, the "correct" inverse scale does not change in the same manner as the parent animation. Let's try to build this intution.

In the following example, we're changing the scale of the parent from 1 to 2 at a _constant_ rate. Every time we change the scale, we apply an inverse scale to the child, plotting the inverse scale along the way. Let's see what that chart looks like:
