...
To invert the size change, we'll use `scale` instead of `translate`:
...
Here's the animation again. Try changing the final size of the square and see if you notice anything about how the distance changes:
...
It turns out that when an element changes size, the distance that element travels will change too! In particular, if an element gets smaller, it has to travel further, and if it gets bigger, it has to travel closer (to its original position).
