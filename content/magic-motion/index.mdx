...
import { SizeDistanceExample } from "./components/SizeDistanceExample";
...
Hmm, that looks a little off. What's going on here? If we pause the animation just before the play step, we can see that something went wrong in the inverse step:

```
broken position and size animation inverse
```

It looks like the square isn't quite lining up with its original position.

### Size Affects Distance Travelled

Let's try to figure this out.

When we combine changes in position and size, we're performing two separate transformations in the inverse step — a translation and a scale. If we take a look at those transformations individually, it becomes pretty clear how the square ended up where it did:
...
You might have noticed it from looking at the first animation, but the reason why the original approach doesn't work is because the transform origin ends up misaligned:

By default, the transform origin of the square will be its center—this causes the misalignment that we see above. But if we change the transform origin to the left edge, notice that our original inverse implementation now works:

You might be thinking then, _"why go to the trouble of using the center at all? Can't we just change the transform origin?"_ Well, what if we're inverting to the _right_ instead? Would using the left edge still work?
