...
show that the text change in the tokenizer visual is a layout change?
```

Another example of a layout change is changing `justify-content` because it causes that element's _children_ to change positions:

```
justify content change
```
...
```
showing transform not doing anything
```
...
This visualization might help clear things up. Notice that the grey boxes also look like they're animating, even though we only transition the blue box:

```
showing recalculation
```

This is made possible because the browser recalculates the position of the grey boxes every time the blue box changes size.
...
FLIP, which stands for **F**irst, **L**ast, **I**nverse, **P**lay, is the underlying technique that powers Framer Motion's layout animations. It allows you to emulate changes in "slow" layout properties like `width` and even lets you animate "un-animatable" CSS properties like `justify-content`.
...
In the **First** step, we measure the position of the element we're animating _before_ any layout changes has happened:
