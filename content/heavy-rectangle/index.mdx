...
Assuming the length of the **hypotenuse** (the blue line in the first two figures above) is 1, then:

1. `sin(angle)` gives us the length of the edge **opposite** to the angle;
2. `cos(angle)` gives us the length of the edge **adjacent** to the angle;
3. `tan(angle)` gives us **how much longer** the opposite edge is compared to the adjacent edge, with `1` meaning the two edges are the same length.
...
1. If you know the length of any two sides, you can calculate the corresponding angle using the _inverse_ trigonometry functions: `asin`, `acos`, and `atan`;
2. If you know the angle and the length of one side, you can calculate the length of any of the other sides.

### Calculating the New Coordinates

If we squint a bit, we can see that the rotated right edge is actually a right-angle triangle:
