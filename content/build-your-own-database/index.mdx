...
Notice that because we're working on a new file, the existing file can still accept new records, like the keys `014` and `006` in the animation above.

Something doesn't look right though. Those new keys we just added weren't copied over! What should we do? We could run the compaction process again, but what if new records come in during _that_ process?
