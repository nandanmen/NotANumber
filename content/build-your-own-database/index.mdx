...
How do we make sure the file doesn't grow indefinitely?

</ProblemStatement>

Because we're using an append-only file, we need some mechanism to periodically "shrink" the file so it doesn't eventually take over our entire hard drive.
...
Here, our database file contains six records, but only two are active—the rest are either deleted or contain stale data. This means if we can periodically trim outdated data, we can keep the file size small!
