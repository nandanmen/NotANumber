import type { DatabaseRecord } from "./AppendOnlyFile";
...
export const createRandomFileRecord = (
  existingRecords: Record[] = []
): Record => {
  return {
    value: createRandomRecord(existingRecords.map(({ value }) => value[0])),
  };
};

export const createRandomFileRecords = (len: number): Record[] => {
  return createRandomRecords(len).map((value) => {
    return { value };
  });
};
