...
Well, they're both written in Framer Motion, but more importantly, they all **take advantage of the React `key` prop**. That's right — the same `key` prop that you use to suppress the React warning when writing loops:
