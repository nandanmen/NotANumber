...
As mentioned in [the last post](./01-how-does-a-compiler-work), a compiler has four distinct phases in its pipeline: tokenization, parsing, transformation, and code generation. The **tokenizer** is the piece of code responsible for handling, you guessed it, the tokenization phase.
...
Corresponds to the following six tokens:
...
In this post, we'll start by going through a small tokenizer that can recognize all the tokens of the following snippet:
...
And then we'll finish up with a little assignment for _you_!

## Single Character Tokens

Let's begin by trying to parse out the simplest tokens first - the ones that are only one character long. In our `console.log` example, that would be the dot and parentheses tokens:
...
How would you implement the `isSingleCharacterToken` function? Try to implement the function in the editor below, and check the output against what's expected to see if you're correct!

<Figure size="xl">
  <Editor test={{ input: ['console.log(message)'], expected: [".","(",")"] }}>
...
One way to implement this is to define a set of the known single character tokens and then checking if the given character is in that set:
...
It's not super useful to return only the character that corresponds to the token. Instead, let's change things up so we'll return the _type_ of the token as well.

Using the above `console.log` as an example, instead of returning this:

```js
tokens = ['.', '(', ')']
```

We want to return this instead:

```json
[{ "type": "Dot" }, { "type": "LeftParen" }, { "type": "RightParen" }]
```

<Callout label="Is this really necessary?">

Bear with me here! This probably looks a bit over-engineered at the moment, but it'll make more sense why this is a good idea when we get to the more complicated tokens later on.

</Callout>

Give it a shot and try to refactor the code from before to return these objects instead:

<Figure size="xl">
  <Editor
    test={{ 
      input: ['console.log(message)'],
      expected: [
        { type: "Dot" },
        { type: "LeftParen" },
        { type: "RightParen" }
