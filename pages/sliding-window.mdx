        ...
        <ul tw="list-none text-center w-1/2 mx-auto">
          {Object.entries(state.patternFrequencies).map(([key, val]) => (
            <li key={key} tw="flex mb-1 font-semibold items-center relative">
              {getLast(key, val) ? (
                <motion.p
                  tw="absolute -right-8 text-gray-500 dark:text-gray-200"
              ...
              <p tw="flex items-center justify-center self-stretch flex-1 font-mono text-sm text-gray-500 bg-gray-100 rounded-md dark:bg-gray-700 dark:text-white">
                {val}
              </p>
            </li>
          ))}
