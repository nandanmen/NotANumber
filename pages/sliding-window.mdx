...
For this problem, we are given an array `arr`, and a subarray size `k`, and we are asked to find the _average_ of _each_ subarray in `arr` of size `k`. So given an array like `[1, 2, 3, 4]` and a subarray size `k = 2`, the correct answer would be `[1.5, 2.5, 3.5]` because:

1. `[1, 2, 3, 4]` has three subarrays of size 2, namely `[1, 2]`, `[2, 3]`, and `[3, 4]`.
2. Taking the average of each subarray we get `[1.5, 2.5, 3.5]`.

How do we create an algorithm to solve this problem? Generally speaking, we would have to (1) divide up the array into subarrays of size `k`, then (2) take the average of each, and finally (3) put them together.
