...
Let's start by taking a look at the AST for the `require` way of importing modules:

<EmbeddedSandbox
  initialCode={`const myModule = require('./my-module')`}
/>

Now let's compare that to the `import` way:

<EmbeddedSandbox
  initialCode={`import * as myModule from './my-module'`}
/>

Using `require`, importing a module is no different than calling a function. This means that, unlike the `var` example from before, we can't simply point to a `Require` node type — we have to check for ourselves if the function call is a call to `require`.

Looking at the AST for just the `require` call, we find that we can identify the name of the function being called using the `CallExpression` node type and checking its `callee` property.

<EmbeddedSandbox
  initialCode={`require('./my-module')`}
  depth={4}
/>
