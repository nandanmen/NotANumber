...
I learned a _ton_, and in the next little bit I want to talk about how exactly it works. In the process, we're going to build our own mini-debugger using Babel's APIs. Let's get started!
...
At a high level, the app is structured as a pipeline with two parts, the **code transpiler** and the **code runner**, all wrapped inside a React UI. When a user types in code in the editor on the left, the code is sent into the pipeline and the React app receives back a list of debug states, shown on the panel on the right.

Within the pipeline, the transpiler transforms the code into code that is "debuggable", and the runner evaluates that code and passes on the result back to the React app.
...
**Wouldn't it be nice if you can generate these save calls?** Sure enough, we can, and we're going to explore exactly how in the next section.

### Where to put it?

Before getting into that though, we have another problem. If we're generating these save calls automatically, _where_ in the function should we put them?

I messed around with ways to automatically inject these calls, but ultimately I felt it made more sense for the users themselves to specify where they want to debug. I wanted this indicator to be easy to write, otherwise we're back to the original problem. In the end, I landed on using the [debugger](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/debugger) keyword as the indicator.

As a whole, the transpiler turns any `debugger` statement into a save call like `variables.push({ ... })`. Here's a demo of the final product:
...
In the next section, we'll get started on building one ourselves!
