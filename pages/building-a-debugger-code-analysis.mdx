...
To answer the original question, **we're not going to convert the code into an AST ourselves**. Rather, we're going to leverage Babel's compiler to do this part of the process for us. The only thing we're going to concern ourselves with is in modifying the AST.
