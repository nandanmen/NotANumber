...
import TraverseTree from '../components/building-a-debugger-code-analysis/TraverseTree'
...
The _de facto_ way of representing code uses something called an **abstract syntax tree**, or AST for short. In an AST, source code is broken apart and grouped into "nodes" based on the structure that they represent.
...
To solve this problem, we're going to take advantage of [the Babel library](https://babeljs.io/).
...
We're going to leverage Babel's compiler here to convert our code string into the AST and the AST back into code. The only thing we're going to concern ourselves with is in modifying the AST.
...
To transform an AST, you have to _traverse_ through the tree, modifying each node one at a time as you visit them. It might look something like this:

<Widget>
  <TraverseTree />
</Widget>

The problem with this approach is that you're coupling the traversal of the tree with modifying the tree. If you want to change the tree differently, you have to change the traversal algorithm!

### The Visitor Design Pattern

The visitor design pattern solves this problem by taking the transformation logic out of the traversal algorithm. Here's how it works.

First, we define a visitor object. It has **node types** as keys and **handler functions** as values. The idea is simple — if the node type matches one of the types we defined in the object, then call the handler function.
...
Next, we modify the traversal algorithm to check in this visitor object for what to do. If there's a match, great — we call the handler function with the current node. Otherwise, no problem — we'll move on to the next node.

<Widget>
  <TraverseVisitor />
</Widget>

Great! We've successfully decoupled the traversal of the AST with the transformation of the AST. [The real implementation](https://github.com/babel/babel/blob/10f4d08efbba49b4085f0756b59c835f97783fa7/packages/babel-traverse/src/index.ts#L41) is a bit more complex, but the core concept still holds. Nevertheless, the visitor pattern is especially useful for libraries like Babel because plugin authors don't have to worry about _how_ the tree traversal works.

### Paths

Admittedly, I lied a bit in the last section. A visitor in a Babel plugin doesn't take in the current node, but rather the current _path_.
...
Cool. With both ASTs in hand, we can get started with building our plugin. We'll start by picking a node type to target in our visitor. Keep in mind that we're going to use this node to build the import statement, meaning we would need the following information:

- The path to the module we're importing, and
- The name we're assigning to the imported module.

As it happens, the `VariableDeclaration` node type is perfect for this. Here's the start of our plugin:
