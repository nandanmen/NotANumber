...
<Placeholder name="formula visual" />
...
But the correct address is 6, since the number "10" takes up 4 bytes of space. Likewise, if we use 4, we'll overshoot the address, while if we use 1, we'll undershoot the address.
...
Instead of storing the element directly, we store a box _containing_ the element we want to store:

![](/box.png)

We'll also need to add a tag that signifies what type of value the box contains. This allows the user of the array to know what type it's getting back.

All in all, our boxed array will look something like this:
