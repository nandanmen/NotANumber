...
This is an array: `[]`. An array represents an ordered list of items. Each item in the array has an associated _index_. Using the index, the array knows exactly where that item is within the array — it doesn't have to look for it:

<Placeholder name="searching vs index lookup" />

In JavaScript, arrays are even more powerful — they can grow in size, and can even contain multiple types of values:
