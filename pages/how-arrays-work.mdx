...
This is an array: `[]`. An array represents an ordered list of items, where each item has an associated _index_. Using the index, the array knows exactly where that item is within the array — it doesn't have to look for it:

<Figure>
  <IndexLookup items={[1, 2, 3]} />
</Figure>

In many programming languages, that's typically all that arrays can do. In JavaScript however, arrays can also grow in size and contain different types of values:

<Placeholder name="dynamic arrays" />

That's great and all, but have you ever wondered **how does it all work?** What's going on "under the hood" when you lookup an item in an array? What makes that operation so fast?

In this post, we're going to dive into the inner workings of an array by building one ourselves, with the help of a few memory primitives.

## Memory API

Before we get too deep into making our own array, I want to talk briefly about computer memory. Throughout this post, I'm going to be using a fake memory object that mimics how computer memory actually works.

This memory object has four methods:
...
If you've written code in C or C++ before, some of these methods might be familiar to you; if not, that's okay too — we'll go through each one in this section.

As a starting point, you can think of memory as a super long array of bytes where each byte has an address. Using the address, you can read from and write to memory:

<Placeholder name="read from memory" />

<Placeholder name="write to memory" />

### Protection

Memory is "gate keeped" by the hardware and operating system (OS) — you can't read _nor_ write from memory unless both the hardware and OS lets you\*.
