...
We have to do a few tricks to get FLIP working in a React component.

The first thing that we want to do is to record the component's position _before_ a layout change. We can use the [`.getBoundingClientRect()`]() function to get an element's current position, but how do we record it every time a layout change happens?

For starters, you can probably assume that if the layout changes, then the component just re-rendered. So let's try that - using `useEffect` to record the position after every render:
...
const Motion = (props) => {
  ...
  }) // omit the dependency array to run every render
