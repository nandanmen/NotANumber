...
import { InverseScaleFormula } from '@/components/magic-motion/InverseScaleFormula'
...
This is because it's expensive for the browser to recalculate where elements go on the page. When you animate something that triggers layout changes, the browser has to do this for _every frame of the animation_.
...
FLIP, which stands for **F**irst, **L**ast, **I**nverse, **P**lay, is the underlying technique that powers Framer Motion's layout animations. It allows you to emulate changes in layout properties like `width` using transforms, giving you the flexibility to change these properties while keeping the performance of transforms.

Here's how it works:

### First

Before we do the layout change, we record the initial position of the element, in terms of `x` and `y` coordinates:

<Flip.First />

### Last

Then we trigger the layout change and record the position of the square again:
...
My thinking was that as long as the timing of the scale correction is the same as the parent animation, this approach should work. Here's what it looks like:

<Placeholder name="first scale correction" />

It's definitely _better_ than no correction, but it still doesn't look right. What's happening here, and how do we make it better?

### The Scale Curve

For starters, the fact that the text is still being stretched out at specific points of the animation suggests that the timing of the inverse transform isn't quite right.

<ProblemStatement>
  What is the right timing for a correct scale correction?
</ProblemStatement>

Let's try to figure this out with a simpler animation.

Here, we have a square that stretches to 5x its size, at a constant rate, over 5 seconds:

<Placeholder name="stretchy square" />
...
Right now the text looks really small because we don't have any corrections applied to the text. To fix this, we need to apply a `scaleX` to the text that cancels out the parent's `scaleX`.

Based on [our formula](#inverse-scale-formula), we need to apply a `scaleX` of `5`:

<Placeholder name="initial inverse" />
