...
<Figure size="lg">
  <Tokenizer name="singleCharacter" input="{ console.log() }" />
</Figure>
...
<Figure size="lg">
  <Tokenizer showKnownTokens={false} />
</Figure>
...
<Callout label="What about `these strings`?">

JavaScript also supports writing strings using backticks like this: \`hello world\`. This type of string is a bit more complicated to tokenize because they allow adding JavaScript syntax inside the string itself - so we're skipping it for now.

</Callout>

To tokenize a string literal, we're going to do something similar to tokenizing identifiers:
...
<Figure size="lg">string parsing</Figure>
...
I'd love to see what you come up with - you can either use the feedback form below or reach out to me [@nansdotio](https://twitter.com/nansdotio) on Twitter.
