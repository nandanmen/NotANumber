...
While, er, that may be true, it's also a bit of an oversimplification. You see, understanding this attribute's inner workings ended up being a huge boon to my front-end skills; it allowed me to do things I never thought possible, like making this bendy square animation:
...
While the `d` attribute may look like some magical incantation, it's actually a **series of commands** that tell the browser how to draw the path. This is a bit more obvious if we clean up the `d` attribute a little bit:
...
This post is a deep dive into the `d` attribute and all the commands that it supports. It's the post I wish I had when I first learned about SVG paths!
