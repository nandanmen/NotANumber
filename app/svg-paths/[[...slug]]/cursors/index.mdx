...
- For **absolute commands**, the cursor will end up at the command's `x` and `y` values. For example, `L 10 10` will move the cursor to `(10, 10)`.
- For **relative commands**, the cursor will end up at the cursor's current position _plus_ the command's `dx` and `dy` values. For example, if the cursor is currently at `(10, 10)`, then `l 10 10` will move the cursor to `(20, 20)`.
