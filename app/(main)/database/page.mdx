...
<TreeControls initialValues={[10, 7, 13]} />

</ScrollGroupSection>

<ScrollGroupSection>

Organizing the data this way makes searching for data really fast since we're essentially cutting the search space in half every time we go down a level:

<TreeControls mode="find" initialValues={[10, 7, 13, 3, 8, 12, 20]} />

</ScrollGroupSection>

<ScrollGroupSection>

The caveat here is the tree must remain _balanced_ for searches to stay fast. Consider this tree where most of the data is on the right side of the tree. If we want to look for the key `18`, for example, we'd have to go through almost all of the nodes in the tree:
...
If we move things around so that the tree is more balanced, we can search for the key `18` in a lot less time:
...
### Persisting a Binary Tree

How do we begin to replicate the binary tree structure on disk? For starters, this is what a binary tree looks like laid out in memory:

```
memory tree
```

Each node in the tree has a _pointer_ to its left and right children. In a file, we could potentially replicate this structure by keeping track of the _offsets_ of each node's children:

```
file tree with offsets
```

While traversing the tree, we can move to a node's left or right child by "jumping" to that child's offset in the file. For new nodes, we'll append them to the end of the file like we did before in an LSM tree.

---

There are a few issues with this setup.

First, appending new nodes to the end of the file means **the distance between a node and its children grows as we add more and more nodes.**

```
growing file tree distance
```

This isn't necessarily an issue with in-memory trees but it is when you put the tree on disk; with persistent storage, data that is far from each other takes longer to access.

Second, **rebalancing is very expensive.** Rebalancing a tree means moving around a lot of data, which is slow when you're working with persistent storage.

```
rebalancing tree on disk
```
...
The reason why we have these issues is because of the differences between accessing data in memory and on disk.

First, **accessing data from memory is a _lot_ faster than accessing data from a disk**. In the time that it takes to read 1MB of data from disk, we can read approximately 275MB of data from memory.

When working with storage devices, we'll see a ton of performance gains by minimizing the amount of data we need to read from disk.

</ScrollGroupSection>

<ScrollGroupSection>

Second, **persistent storage is not byte-addressable**.

Volatile memory like RAM are byte-addressable which means the smallest unit of data that we can access from memory is a byte. Each byte of storage has a unique address that we can use to read or write data.

On the other hand, the smallest unit of data that we can access from non-volatile storage is typically around 4KB (4 kilobytes, or 4096 bytes) in size, usually referred to as a _block_. In other words, if we want to update a few bytes of data, we have to update the _entire_ 4KB block that contains that data.

</ScrollGroupSection>

<ScrollGroupSection>

Lastly, **persistent storage prefers sequential access**.

When we need to read or write multiple blocks of data, it's usually a lot faster to read or write them _continuously_ than it is _randomly_.

For example, if we need to update blocks 10, 11, and 12, it's faster to update them in order (i.e. block 10 first, then block 11, then block 12) than it is to update them in a random order (e.g. block 10 first, then block 12, then block 11).

This doesn't affect more modern storage devices (like SSDs or NVMe drives) as much as hard drives, but hard drives are still by and large the cheapest way to store large amounts of data—so it's still something we need to consider.
...
### Aside: Why hard drives prefer sequential access

A hard drive is made up of a stack of spinning disks, with each disk divided into _tracks_. Each track is further divided into _sectors_, which are the smallest unit of data that can be read or written to a hard drive.

```
hard drive read/write example
```

To read or write data, a _disk head_ has to _physically_ move over the sector. This involves positioning the disk head over the correct track (a process known as seeking) and waiting for the sector to move below it.

Seeking is the primary bottleneck in accessing data from a hard drive. In the time it takes to seek (2ms), we can read over 666MB of data from memory.
