...
import { FileDatabaseVisualizer, FileDatabaseControls } from './_components/mutable-database'
...
The typical way to store any kind of data in a computer is to use a <Annotation type="underline"> _file_ </Annotation>. When we want to store data, we add the key-value pair to the file:

<FileDatabaseControls mode={["add"]} mutable />

And when we want to look for a specific key, we iterate through the pairs to see if there's a matching key:
