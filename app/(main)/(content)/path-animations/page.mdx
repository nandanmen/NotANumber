...
**Heads up!** This post will not cover how you go about drawing the shape in the first place. If you're not familiar with path commands and the `d` attribute, check out my post on [SVG paths](/svg-paths).
...
Before we get into the animations themselves, we first need to understand two important SVG properties: `stroke` and `fill`.

To draw a path, the browser first executes the commands in the `d` attribute, in sequential order, creating a sort of "skeleton" of the shape:
...
Finally, if the path has its `stroke` property set, a line will be drawn along the shape's skeleton, using whatever color `stroke` is set to.

---

### Path-based vs. Fill-based shapes

Since both `fill` and `stroke` can be used to paint a shape with color, it's technically possible to create idential shapes using just fill _or_ just stroke.

```
fill based vs stroke based
```
