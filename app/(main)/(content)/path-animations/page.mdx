...
import { PathDrawing } from './_components/path-drawing'
import { PathLength, PathLengthVisual } from './_components/path-length'

Perhaps the most common of path animations is the path drawing animation:

<Drawing />

This animation looks pretty straightforward, but there's actually quite a bit of nuance required to get it looking and feeling just right. Let's take a closer look!

---

<ScrollGroup figure={<PathDrawing />}>

<ScrollGroupSection>

## How paths are drawn

To best understand how this animation works, we'll need to explore how paths are drawn by the browser in the first place.

All path elements are defined by a `d` attribute: a string of commands that dictate how the path should be drawn:

```html
<path
  d="M4 20C4 20 4 4 7 4C9 4 9 11 11.5 11C14 11 15 6.7 20 8"
/>
```

We won't dive into the details of each command here; all you need to know is that each letter code corrsponds to a drawing operation (e.g. `M` is for move, `L` is for line, `C` is for cubic curve, etc.) and that the numbers following the letter code are the arguments for that operation.

Using the `d` attribute, the browser starts with a "skeleton" of the shape:
...
The `stroke-width` and `stroke-linecap` properties let you change the appearance of the stroke. `stroke-width` sets the thickness of the stroke, and `stroke-linecap` sets the shape of the stroke's ends.
...
Here, we've set `stroke-dasharray` to 5 which means we want dashes of length 5, separated 5 units apart.

</ScrollGroupSection>

<ScrollGroupSection>

If we set `stroke-dasharray` to a value large enough, we can make each dash **as large as the path itself**:
...
Now, the path looks the same as if it didn't have a `stroke-dasharray` at all.

</ScrollGroupSection>

<ScrollGroupSection>

This isn't super interesting on its own, until you remember that the path-sized dashes are separated by _path-sized gaps_. This lets us hide the path if we just move the dash out of bounds.

Thankfully, we can do this using the `stroke-dashoffset` property:
...
And that's the end of the blog post! Or, it would've been—if it wasn't for a few other edge cases we need to think about. Let's take a look at them.

---

### Choosing the right path length

Technically speaking, the animation will work as long as the dash is *at least* as long as the path itself. This means we can set `stroke-dasharray` to some really large number without many noticeable differences:

<PathLength />

You'll find that the larger `stroke-dasharray` value finishes much faster, but other than that, there isn't really a difference between these two checkmarks—at least, at first.

The real problem comes when we try to inverse the animation:

<PathLength inverse />

The checkmark with the larger `stroke-dasharray` has a noticeable delay before it actually starts animating. What's going on here?
