...
2. The router will see the destination is in network 2, and sends it to the router in Network 2;
...
In this little internet, it's possible for every router in the network to know about every other router because there are only three routers. In the real world, this is not the case—there are **hundreds of millions of routers** in the Internet.

To keep things organized, routers in the Internet are organized into smaller networks called **Autonomous Systems** (AS for short). Each AS is owned and operated by some governing body, such as hosting providers (Google, Amazon, Cloudflare), ISPs (Telus, Bell), or universities.

Most traffic in the Internet has to go through several ASes. For example, data between my computer and this website has to go through Telus's AS (my ISP) and Amazon's AS (where the website is hosted).

---

Traffic between ASes is handled by a group of routers called **border routers**. These are routers dedicated exclusively to handling traffic moving in/out its AS.

If the networks in our little internet were ASes for example, you can think of border routers as the routers that connect the ASes to each other:
...
Organizing networks into ASes eases the burden on all the routers in the network. When getting data out of an AS:

- Routers within an AS only need to get traffic to the border routers;
- Border routers only need to get traffic to other ASes;
- Border routers of the other AS can handle routing traffic into the right computer within its AS.

```
some AS visual with border routers
```
