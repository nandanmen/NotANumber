import { BGPPathFindingSequenceList, BGPScrollVisual } from "./_components/bgp";
import { DNSVisual, DNSScrollVisual } from "./_components/dns";
import { Section } from "./_components/grid-snapped-section";
import { LittleInternetScrollVisual } from "./_components/little-internet";

<ScrollGroup figure={<LittleInternetScrollVisual />}>

<ScrollGroupSection>

## A Little Internet

One way to transmit data between two computers is to have them physically connected to each other—be it through cables or some form of wireless technology. To send data, a computer only needs to send it along the physical connection.
...
<ScrollGroup figure={<BGPScrollVisual />}>
...
One way is for router 3 to **announce its existence** to router 1.

<BGPPathFindingSequenceList>

1. When router 3 starts up, it will announce to router 2 that it can receive all addresses starting with `3.x`.
2. Router 2 will update its routing table to say that all `3.x` addresses should be sent to router 3.
3. Router 2 will then announce to router 1 that it can receive all addresses starting with `3.x`.
4. Router 1 will update its routing table to say that all `3.x` addresses should be sent to router 2.

</BGPPathFindingSequenceList>

Router 1 can now send data to router 3!

</ScrollGroupSection>

<ScrollGroupSection>

What about router 3? How does router 3 know about the other 2 routers? Router 2 already knows about router 1, so router 2 can tell router 3 during the initial connection.
...
Let's now add a direct connection between router 1 and router 3.

This time, when router 3 starts up, it will announce to _both_ router 1 and router 2 that it can receive all addresses starting with `3.x`.

</ScrollGroupSection>

<ScrollGroupSection>

Router 2 doesn't know that router 1 and router 3 are directly connected to each other, so it will also announce to router 1 that it can receive all addresses starting with `3.x`.

What should router 1 do in this case?
