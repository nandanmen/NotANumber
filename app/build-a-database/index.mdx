...
Since we want to design data structures that work well with hard drives, let's talk about how hard drives work and how they differ from RAM.

### Hard drives are not byte addressable

If you read my [how arrays work]() post or know a bit about RAM, you'll know that RAM works a lot like an array — you can access individual bytes of data using their address:

```
RAM visual
```

Hard drives are different because they're not **byte-addressable** — you can't access individual bytes of data. Instead, you have to read and write entire **blocks** of data at a time:

```
RAM visual but blocks
```

These blocks are, comparatively speaking, quite large — most modern hard drives have blocks that are 4KB (4 kilobytes, or 4096 bytes) in size.

### Hard drives are much slower than RAM

- Fast RAM can do ~20GB/s
- Fast SSDs can do ~3GB/s
- Fast hard drives can do ~200MB/s
