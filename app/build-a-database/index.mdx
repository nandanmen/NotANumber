...
**What about updates and deletes?** Probably the most intuitive way to go about this is to update the file and then either replace or delete the old key-value pair:

<AppendOnlyFile mode={["add", "update", "delete"]} />

While this makes sense, it doesn't actually work that well in practice. For starters, it's rather complex—for every key we want to update or delete, we have to:

1. Search for the key in the file;
2. Either replace or delete the key-value pair;
3. _Move_ the position of every following record to fit the new record.

Step 3 in particular is problematic because it's painfully _slow_. To understand why, we'll need to take a detour into **how computers store data**.
